      ...
      <Link href={`/product/${product.id}`} className="group block">
        ...
        >
          <div className="relative aspect-[4/5] overflow-hidden bg-mist dark:bg-white/5">
            ...
            {product.featured && (
              <span className="absolute right-3 top-3 rounded-full bg-navy/90 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-white">
                Destacado
              </span>
            )}
            <button
              ...
              className="absolute bottom-3 right-3 flex h-8 w-8 items-center justify-center rounded-full bg-white/90 backdrop-blur transition hover:scale-110 dark:bg-ink/70"
            >
              <Heart
                className="h-4 w-4"
                strokeWidth={1.75}
